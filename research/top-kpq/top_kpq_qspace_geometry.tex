...
The one-sign move $(i,j)\mapsto(i-1,j+1)$ is an elementary unit-transfer operation closely related to classical majorization theory \cite{MarshallOlkinArnold2011}. Rational-normal-curve toric ideals also admit combinatorial and chip-firing interpretations \cite{KarkiManjunath2023}. We therefore do not claim invention of unit-transfer dynamics or of rational-normal-curve normal forms. The contribution asserted here is the exact signed even-cycle synthesis with the frozen $(K,P,Q)$ coordinates, terminal formula, Q-space simplex, and metric-affine action.
...
\]
For every residue $e$, the two representatives $e$ and $2H-e$ have the same parity, so
\[
\D(e)\equiv e\pmod2.
\]
Therefore
\[
L\equiv\sum_t e_t\equiv P\equiv\D(P)\pmod2.
\]
The circular metric also satisfies the triangle inequality, hence $L\ge\D(P)$. Consequently
\[
...
\begin{proof}
The invariants give $m=K-1$ and $L=2Q+\D(P)$. Any irreducible terminal has the stated shape. For such a terminal,
\[
L=a+b+hH
\]
and, for some integer $q$,
\[
P=a-b+hH+2Hq,
\]
where $P$ is the chosen representative in $\{0,\ldots,2H-1\}$. Hence
\[
\frac{L+P}{2}=a+H(h+q),\qquad
\frac{L-P}{2}=b-Hq.
\]
Reduction modulo $H$ therefore forces the stated residues $a$ and $b$ uniquely in $[0,H)$. Then
\[
h=\frac{L-a-b}{H}
\]
is forced and is a nonnegative integer for every reachable class. Finally the movement count gives
\[
z=m-h-[a>0]-[b>0].
\]
Thus no two distinct irreducible terminals can share the same $(m,P,L)$, hence not the same $(K,P,Q)$.
\end{proof}
...
Fix a reachable $(P,Q)$ class and write $d=\D(P)$. Represent $P$ in $\{0,\ldots,2H-1\}$ and choose the endpoint direction to be positive when $0\le P\le H$ and negative when $H<P<2H$; at $P=0,H$ this is a convention only. Let $A$ denote total ordinary magnitude in that chosen endpoint direction, $B$ the total ordinary magnitude in the opposite direction, and $h$ the number of half-turn movements.
...
T=r+s.
\]
For every member of the class, the oriented signed displacement relation gives
\[
A-B+hH\equiv d\pmod{2H},
\]
while
\[
A+B+hH=L=2Q+d.
\]
Adding and subtracting these congruences yields
\[
A\equiv Q+d\pmod H,\qquad B\equiv Q\pmod H.
\]
Because $A,B\ge0$, there are unique nonnegative integers $\eta^+,\eta^-$ such that
\[
A=a+H\eta^+,\qquad
B=b+H\eta^-.
\]
...
\eta^++\eta^-+h
=\frac{L-a-b}{H}
=\left\lfloor\frac{Q+d}{H}\right\rfloor+\left\lfloor\frac QH\right\rfloor
=T.
\]
Thus, with $\eta_H=h$,
\[
...
\begin{proof}
Necessity is immediate: each ordinary movement has magnitude at most $H-1$, so total directional magnitude $A$ requires at least $c_H(A)$ ordinary slots, total opposite magnitude $B$ requires at least $c_H(B)$ slots, and the $h$ half-turns require $h$ further slots.

For sufficiency, any positive integer $S$ can be written as a sum of exactly $c_H(S)$ integers in $\{1,\ldots,H-1\}$: take as many $H-1$ terms as possible and one final remainder when needed. Apply this independently to $A$ and $B$, add the prescribed $h$ half-turns, and fill any unused slots with zero movements. The resulting multiset has the required $m,A,B,h$, hence the required $P$ and $L$, and therefore the required $(K,P,Q)$. By Theorem~\ref{thm:complete}, it belongs to the target collision class.
\end{proof}

\section{Minimum realizable order}
For $0\le a,b<H$ and $\eta\ge0$,
\[
c_H(a+H\eta)\ge \eta+[a>0],
\]
and similarly for $b$: if $a=0$ this is immediate, while if $a>0$ then
\[
\frac{a+H\eta}{H-1}=\eta+\frac{a+\eta}{H-1}>\eta,
\]
so the ceiling is at least $\eta+1$. Hence every admissible simplex point satisfies
\[
m\ge \eta^++\eta^-+\eta_H+[a>0]+[b>0]
=T+[a>0]+[b>0].
\]
The canonical terminal attains equality by taking $\eta^+=\eta^-=0$ and $\eta_H=T$. Therefore
\[
m^\ast=T+[a>0]+[b>0],
\]
and the minimum selected-position count is
\[
\boxed{
K^\ast=1+T+[a>0]+[b>0].
}
...
Consider a uniform affine bijection of the cyclic movement set
\[
...
The metric-affine movement symmetry group is exactly
\[
\boxed{
V_4=\{I,C,A,AC\},
}
...
For $m\ge1$, the uniform diagonal $V_4$ action commutes with arbitrary permutations of the $m$ movement coordinates, and the two actions intersect trivially. Therefore:
