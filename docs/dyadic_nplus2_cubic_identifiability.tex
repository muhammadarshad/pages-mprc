...
\]
Then \(K\) is a binary \([n+2,2]\) relation code. Equivalently, (d(K)\ge5) means that the parity support augmented by the origin is a binary Sidon set. For \(3\le n\le17\) we prove an exact classification:
\[
\boxed{\quad
...
For \(3\le n\le17\), the parity support is always rigid under the stated spanning/distinctness assumptions. Over \(R_m\), \(m\ge2\), the decomposition is globally identifiable if and only if the binary relation code has minimum distance at least five.
...
\]
the constant moment agrees as well. Hence, for \(n\ge4\),
\[
...
For \(3\le n\le13\), the previous Reed--Muller low-weight argument applies: Kasami--Tokura's classification below \(2d=32\), together with the affine-span bounds of the corresponding normal forms, rules out every nonzero support because \(X\) spans \(\F_2^n\).

It remains to treat \(14\le n\le17\). Let
\[
F=\supp(z),\qquad c=|F|.
\]
Then \(c\le2(n+2)\le38\). Form the \(n\times c\) binary matrix \(M_F\) whose columns are the distinct points of \(F\), and let
\[
C_F=\operatorname{RowSpan}(M_F).
\]
Orthogonality of \(z\) to all degree-one, degree-two, and degree-three monomials says exactly that every row of \(M_F\) has even weight, every pair of rows has even overlap, and every triple of rows has even overlap. Thus \(C_F\) is an all-even triorthogonal subspace.

Let
\[
H_F=\langle C_F,\mathbf1_c\rangle.
\]
Because \(C_F\) is self-orthogonal and all of its rows have even weight, adjoining \(\mathbf1_c\) preserves triorthogonality. Hence \(H_F\) is a unital triorthogonal subspace. The columns remain distinct, and the all-one coordinate ensures that no column is zero.

Nezami and Haah classified all unital triorthogonal subspaces with no repeated columns and fewer than \(40\) columns \cite{NezamiHaah2022}. Their exhaustive Table II gives the following maximal dimensions:
\[
\begin{array}{c|rrrrrrrr}
c&16&24&28&30&32&34&36&38\\ \hline
h(c)&5&7&8&9&10&9&10&11.
\end{array}
\]
No other nonzero column counts below \(40\) occur in their classification. Since
\[
\dim\langle F\rangle=\rank M_F\le\dim H_F\le h(c),
\]
we obtain a direct rank bound on every possible collision support.
...
t\ge c/2.
\]
Thus at most \(n+2-c/2\) points of \(X\) lie outside \(F\), and therefore
\[
\dim\langle X\rangle
\le h(c)+n+2-c/2.
\]
...
Consequently
\[
\dim\langle X\rangle<n,
\]
contradicting the spanning hypothesis. Hence \(z=0\).

Therefore \(Y_{\rm odd}=X\). Since \(Y\) contains exactly \(n+2\) terms and all \(n+2\) elements of \(X\) must occur with positive odd multiplicity, each occurs exactly once.
\end{proof}

\begin{remark}
The first unresolved dimension for the global-support argument is \(n=18\). Then
\[
2(n+2)=40,
\]
so a collision can have a \(40\)-point support, just outside the exhaustive \(c<40\) triorthogonal classification used above. All collision supports of size below \(40\) remain excluded by the same argument.
\end{remark}
...
The global parity theorem is proved here for \(3\le n\le17\). At \(n=14\), a symmetric difference may have weight \(32=2d\), outside the strict below-\(2d\) Kasami--Tokura classification used in the proof. Extending the global classification beyond this point therefore requires a separate analysis of weight-\(32\) supports and their possible interaction with a spanning \(n+2\)-point frame.

The relation-code/Jacobian theorem itself is not restricted to \(n\le17\); Theorem~\ref{thm:jac} applies whenever the frame has \(n+2\) distinct nonzero spanning columns. What changes beyond \(n=17\) is the global recovery of the parity support.
