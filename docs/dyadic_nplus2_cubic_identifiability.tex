...
\]
Then \(K\) is a binary \([n+2,2]\) relation code. For \(3\le n\le13\) we prove an exact classification:
\[
\boxed{\quad
...
For \(3\le n\le13\), the parity support is always rigid under the stated spanning/distinctness assumptions. Over \(R_m\), \(m\ge2\), the decomposition is globally identifiable if and only if the binary relation code has minimum distance at least five.
...
Let \(3\le n\le13\). Let \(X\subset\F_2^n\setminus\{0\}\) consist of \(n+2\) distinct points spanning \(\F_2^n\). If a multiset \(Y\) of \(n+2\) vectors in \(\F_2^n\) satisfies
\[
...
The case \(n=3\) is immediate because square-free Boolean monomials of degrees at most three, together with the constant, form a basis of all Boolean functions on \(\F_2^3\). The cases \(n=4,5\) follow directly from the weight spectra of \(RM(0,4)\) and \(RM(1,5)\). Assume \(6\le n\le13\). Then
\[
\wt(z)\le 30<32=2d,
\qquad d=16.
\]
Kasami--Tokura's classification of Reed--Muller words below twice the minimum distance implies that every nonzero such \(z\) has weight
\[
16,\quad24,\quad28,\quad\text{or}\quad30,
\]
with the following support-span bounds.

A weight-\(16\) word is a minimum word, hence the indicator of an affine \(4\)-flat; its support has linear span dimension at most \(5\).

For the other weights, specialize the two Kasami--Tokura normal forms to order \(\nu=n-4\). Their supports lie in affine subspaces of dimension at most \(6\), \(7\), and \(8\), respectively, giving the conservative linear-span bounds
\[
\begin{array}{c|cccc}
\wt(z)&16&24&28&30\\ \hline
\dim\langle\supp(z)\rangle&\le5&\le7&\le8&\le9.
\end{array}
\]
More explicitly, weight \(24\) comes from the quadratic-pair normal form with two hyperbolic pairs; weight \(28\) comes either from three hyperbolic pairs or from the two-product normal form with product length three; and weight \(30\) comes from the two-product normal form with product length four.
...
n+2-w/2
\]
points of \(X\) lie outside \(F\). Combining this with the support-span bounds gives
\[
\dim\langle X\rangle
\le
\begin{cases}
...
\end{cases}
\]
Every case contradicts \(\langle X\rangle=\F_2^n\). Therefore \(z=0\), so \(Y_{\rm odd}=X\). Since \(Y\) has exactly \(n+2\) terms and each of the \(n+2\) elements of \(X\) must occur an odd positive number of times, every one occurs exactly once.
\end{proof}

\begin{remark}
The simple weight-gap proof stops at \(n=9\), but the Kasami--Tokura normal forms control the newly reachable weights \(24\), \(28\), and \(30\), extending rigidity through \(n=13\). At \(n=14\), weight \(32=2d\) becomes reachable and this below-\(2d\) argument no longer applies.
\end{remark}
...
The global parity theorem is proved here for \(3\le n\le13\). At \(n=14\), a symmetric difference may have weight \(32=2d\), outside the strict below-\(2d\) Kasami--Tokura classification used in the proof. Extending the global classification beyond this point therefore requires a separate analysis of weight-\(32\) supports and their possible interaction with a spanning \(n+2\)-point frame.
