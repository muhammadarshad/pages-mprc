...
c=16,
\]
because \(z\) is orthogonal to the constant word and hence has even weight.
...
\begin{remark}
The parity-rigidity theorem has no upper dimension cutoff. The proof uses only Reed--Muller duality and minimum distance, the affine-flat form of minimum-weight words, and Lemma~\ref{lem:triobound}. The earlier finite-dimensional proof through \(n=17\) remains useful as independent regression evidence but is no longer needed for the theorem.
\end{remark}
