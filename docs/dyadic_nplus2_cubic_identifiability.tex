...
Let \(3\le n\le17\). Let \(X\subset\F_2^n\setminus\{0\}\) consist of \(n+2\) distinct points spanning \(\F_2^n\). If a multiset \(Y\) of \(n+2\) vectors in \(\F_2^n\) satisfies
\[
...
\begin{proof}
Let \(Y_{\rm odd}\) be the set of points occurring with odd multiplicity in \(Y\). Over \(\F_2\), the tensor of the multiset \(Y\) equals the tensor of \(Y_{\rm odd}\), and
\[
...
z=1_X+1_{Y_{\rm odd}}.
\]
Equality of the cubic tensors gives equality of every nonconstant square-free Boolean moment of degrees one, two, and three; the parity of the cardinalities supplies the constant moment. Hence
\[
z\in RM(3,n)^\perp.
\]
For \(n\ge4\), Reed--Muller duality gives
\[
z\in RM(n-4,n).
\]
Also
\[
\wt(z)\le |X|+|Y_{\rm odd}|\le2(n+2).
\]

For \(n=3\), square-free Boolean monomials of degrees at most three, together with the constant, form a basis of all Boolean functions on \(\F_2^3\), so \(z=0\). Assume \(4\le n\le17\). Then
\[
\wt(z)\le 2(n+2)\le38<40=\frac52 d,
\qquad d=16.
\]

Kasami--Tokura--Azumi classify all binary Reed--Muller words of weight below \(2.5d\). Specializing their classification to \(RM(n-4,n)\), and using the explicit affine representatives tabulated by Nezami--Haah, every nonzero word with weight below \(40\) has weight
\[
16,\ 24,\ 28,\ 30,\ 32,\ 34,\ 36,\ \text{or }38.
\]
Moreover the support of each such word is contained, after affine change of variables and restoration of any removed linear factors, in an affine subspace whose linear span has dimension bounded as follows:
\[
\begin{array}{c|rrrrrrrr}
\wt(z)&16&24&28&30&32&34&36&38\\ \hline
\dim\langle\supp(z)\rangle
&\le5&\le7&\le8&\le9&\le10&\le9&\le10&\le11.
\end{array}
\]
For weights \(16,24,28,30\) these bounds follow from the classical Kasami--Tokura normal forms. For weights \(32,34,36,38\), the complete low-weight representatives in the Kasami--Tokura--Azumi range use at most \(9,8,9,10\) effective variables respectively; their supports therefore have linear span at most \(10,9,10,11\).

Let \(F=\supp(z)\), \(w=|F|\), and \(t=|X\cap F|\). Since
\[
Y_{\rm odd}=X\triangle F,
\]
we have
\[
|Y_{\rm odd}|=|X|+w-2t\le|X|,
\]
so
\[
t\ge w/2.
...
n+2-w/2
\]
points of \(X\) lie outside \(F\). If \(s(w)\) denotes the corresponding support-span bound above, then
\[
\dim\langle X\rangle
\le s(w)+n+2-\frac w2.
\]
For the eight possible weights this gives, respectively,
\[
n-1,\ n-3,\ n-4,\ n-4,\ n-4,\ n-6,\ n-6,\ n-6,
\]
all strictly less than \(n\). This contradicts
\[
\langle X\rangle=\F_2^n.
\]
Hence \(z=0\), so \(Y_{\rm odd}=X\). Since \(Y\) contains exactly \(n+2\) terms and every one of the \(n+2\) elements of \(X\) must occur an odd positive number of times, every one occurs exactly once.
\end{proof}

\begin{remark}
The proof uses the strict Kasami--Tokura--Azumi range \(\wt<2.5d=40\). Since
\[
2(n+2)\le38
\]
for \(n\le17\), every possible collision word is covered. At \(n=18\), weight \(40\) becomes reachable and the cited low-weight classification no longer applies directly. Thus \(n=18\) is the first unresolved dimension for the present global-support argument.
\end{remark}
