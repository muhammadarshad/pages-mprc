...
\]
Then \(K\) is a binary \([n+2,2]\) relation code. Equivalently, (d(K)\ge5) means that the parity support augmented by the origin is a binary Sidon set. For \(3\le n\le13\) we prove an exact classification:
\[
\boxed{\quad
...
The relation-code/Jacobian theorem itself is not restricted to \(n\le13\); Theorem~\ref{thm:jac} applies whenever the frame has \(n+2\) distinct nonzero spanning columns. What changes beyond \(n=13\) is the global recovery of the parity support.
